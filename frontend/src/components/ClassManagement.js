...
const ClassManagement = () => {
    ...
    const fetchProfessors = async () => {
        try {
            ...
            // Si es profesor, pre-selecciona su propio ID
    ...
    const handleCreateClass = async (e) => {
        ...
        try {
            const classData = {
                ...
                professor: { id: parseInt(newClass.professorId) } // Enviar como objeto
            };
        ...
        <section>
            ...
            <form onSubmit={handleCreateClass} style={{ marginBottom: '20px' }}>
                ...
                <select
                    ...
                    required
                    disabled={auth.role === 'PROFESOR'} // Profesores no eligen, se asignan a sí mismos
                >
            ...
            {classes.length > 0 ? (
                <ul style={{ listStyle: 'none', padding: 0 }}>
                    {classes.map(cls => (
                        <li key={cls.id} style={{ marginBottom: '10px', backgroundColor: '#F5F5F5', padding: '10px' }}>
                            <span>{cls.name} ({cls.code}) - {cls.description || 'No description'} (Prof: {cls.professor?.name})</span>
                            <button onClick={() => handleDeleteClass(cls.id)} style={{ marginLeft: '10px', backgroundColor: '#EF5350' }}>
                                Delete
                            </button>
                            <button onClick={() => setSelectedClassId(cls.id)} style={{ marginLeft: '10px' }}>
                                Add Student
                            </button>
